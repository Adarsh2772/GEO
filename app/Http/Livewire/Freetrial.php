...
{
    public $packages,$package_id,$branch,$package,$addons,$addonmodules,$validity=4,$validities,$addonfunction,$tabselectionstatus;
    ...
    {
        return view('livewire.freetrial');
    }
    ...
    public function applydiscount()
    {
        ...
        if ($this->validity > 1 && trim($this->coupon_code)) {
            $this->coupon_code = '';
        }
    ...
    public function saveform()
    {
        ...
        if(sizeof($branch) != 0){
            if($branch['status'] == false){
                if ($branch['branch_id'] != 0) {
                    ...
                    $razorpayid = "Free Trial";
                    $razorpay_order_id = "";
                    $razorpay_signature = "";
                    $this->proceedtopayment($razorpayid,$razorpay_order_id,$razorpay_signature);
                    return false;
                }
            }
        }
        ...
        $freetrial = Http::post(config('constants.DEFAULT_URL').'/website/freetrialmail', ['branchdata' => $this->branch]);
    ...
    public function proceedtopayment($razorpayid,$razorpay_order_id,$razorpay_signature)
    {
        ...
        if(isset($data['transaction_id'])){
            ...
            $this->dispatchBrowserEvent('iztoast',["title" => 'Congratulations on your successful DueDeck subscription!', 'refresh' => 1, 'close' => 1, 'reload' => 1]);
