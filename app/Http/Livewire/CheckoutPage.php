...
{
    public $packages,$package_id,$branch,$package,$addons,$addonmodules,$validity=1,$validities,$addonfunction,$tabselectionstatus;
    ...
    public $subtotal,$subtotal_amount,$grandtotal,$CGST,$SGST,$ismoduleperyear=1;
    ...
    public function mount()
    {
        ...
        $this->packages = Http::acceptJson()->get(config('constants.DEFAULT_URL').'/website/getPackages')->collect()['package'];

        $this->defaultmodules = $object['defaultmodules'];
        $this->validities = Http::acceptJson()->get(config('constants.DEFAULT_URL').'/website/getValidity')->collect()->where('validityorder', '!=', 0);
    ...
    public function plusminus($id, $value, $amount, $no_of_count,$noaddon=true)
    {
        ...
        if(array_key_exists($id,$this->addonfunction['addon'])){
            if($value >= 0 || $this->addonfunction['addon'][$id]['quantity'] >= 0){
                ...
                $percountvalue = round($amount/$no_of_count,2);
                if($this->addonfunction['addon'][$id]['isyear'] == 1){
                    $this->addonfunction['addon'][$id]['total'] = (intval($this->addonfunction['addon'][$id]['quantity']) * $percountvalue) * $validity;
                }else{
                    $this->addonfunction['addon'][$id]['total'] = (intval($this->addonfunction['addon'][$id]['quantity']) * $percountvalue);
                }
    ...
    public function saveform()
    {
        $rules = [
            ...
            'contact_person_no' => 'required|digits:10|regex:/^[1-9][0-9]{9}$/|max:10',
            ...
            'contact_no' => 'required|digits:10|regex:/^[1-9][0-9]{9}$/|max:10',
            'email' => 'required|max:50|email',
        ];

        $this->branch['created_by'] = 1;
        $this->branch['updated_by'] = 1;
        $this->branch['contact_person_name'] = $this->branch['name'];
        $this->branch['contact_person_no'] = $this->branch['contact_no'];
        $this->branch['contact_person_email'] = $this->branch['email'];
        ...
        // dd($this->captcha, $this->originalcaptcha);
        ...
        if($this->grandtotal < 1){
            $this->dispatchBrowserEvent('iztoast', ["title" => 'Grand total amount should be greater than 100 paise, i.e. ₹ 1', 'status' => 0]);
            return false;
        }
        ...
        $uniqueId = $this->generateUniqueID(10);
            ...
            $orderData = [
                'receipt'         => $uniqueId,
                'amount'          => (int)$amount, // 39900 rupees in paise
                'currency'        => 'INR'
            ];

        $api = new Api("rzp_live_U88cXlzkto7hqh", "pIgDlhH1xhjSjIjK0hBvqn48"); 

        $orderid = $api->order->create($orderData)->id;

        if(sizeof($branch) != 0){
            if($branch['status'] == false){
                if ($branch['branch_id'] != 0) {
                    $this->branch_id = $branch['branch_id'];
                    $this->dispatchBrowserEvent('razorpay', ['orderid' => $orderid]);
                    return false;
                }
            }
        }
    ...
    public function proceedtopayment($razorpayid,$razorpay_order_id,$razorpay_signature)
    {
        ...
        $paramsdata = [
            ...
            'discount' => $discount,
            'subtotal_amount' => $this->total_amount,
            'cgst' => $this->CGST,
            'sgst' => $this->SGST,
        ...
        if(isset($data['transaction_id'])){
            ...
            $transactionresponse = Http::post(config('constants.DEFAULT_URL').'/website/transactionhistory', [
                'historydata' => $historydata,
            ]);

            $response = Http::post(config('constants.DEFAULT_URL').'/website/applypromocode', [
                'promocode' => $this->coupon_code,
            ]);
            

            $this->confirmorder($page=3);
            $this->dispatchBrowserEvent('iztoast',["title" => 'Congratulations!!! DueDeck Subscription Payment successful.', 'refresh' => 1, 'close' => 1, 'reload' => 1]);
    ...
    function generateUniqueID($length = 10){
        $timestamp = microtime(true); // Get current timestamp
        $randomString = Str::random($length); // Generate a random string
        $uniqueId = md5($timestamp . $randomString); // Hash the combination of timestamp and random string
        return $uniqueId;
    }

    public function confirmorder($page=1){
        $this->checkout = $page;
        $this->dispatchBrowserEvent('iztoast', [ 'refresh' => 1]);
